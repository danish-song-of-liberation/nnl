...
(defmethod initialize-instance :after ((nn intern-fc) &key)
  (let ((input-shapes (input-shapes nn))
	...
	(case (initialize-method nn)
	  (:random 
	    (setf (weights nn) (nnl.init:random-initialize (list output-shapes input-shapes) :from (init-from nn) :to (init-to nn) :requires-grad t))
		
		(when use-bias-p 
		  (setf (bias nn) (nnl.init:random-initialize (list output-shapes) :from (/ (init-from nn) 10) :to (/ (init-to nn) 10) :requires-grad t))))
		  
	  (:xavier/normal
	    (setf (weights nn) (nnl.init:xavier-normal-initialize (list output-shapes input-shapes) (+ input-shapes output-shapes) :requires-grad t))
		
		(when use-bias-p
		  (setf (bias nn) (nnl.math:zeros (list output-shapes) :requires-grad t))))
		  
	  (:xavier/uniform
	    (setf (weights nn) (nnl.init:xavier-uniform-initialize (list output-shapes input-shapes) (+ input-shapes output-shapes) :requires-grad t))
...
(defmethod forward ((nn intern-fc) input-data)
  (let* ((expected-rows (input-shapes nn))  
         (current-shapes (nnl.math:shape input-data))
         (actual-rows (first current-shapes))) 

    (when (/= actual-rows expected-rows)
      (setf (nnl.math.autodiff::data input-data)
            (nnl.magicl:transpose
              (nnl.magicl:get-magicl-type current-shapes nnl.system::*calculus-system*)
              (nnl.math.autodiff::data input-data)))))
...
(defmethod parameters ((nn intern-fc))
  (if (use-bias nn)
